...
set "ICON_PATH=%ASSET_DIR%\1600p.ico"
set "SETTINGS_DEST=%localappdata%\FortniteGame\Saved\Config\WindowsClient\GameUserSettings.ini"
set "SETTINGS_LOCAL=%ASSET_DIR%\GameUserSettings_1600p.ini"
set "REPO_DIR=%MY_DIR%.."

if not exist "%ASSET_DIR%" mkdir "%ASSET_DIR%"

if not exist "%ICON_PATH%" (
    if exist "%REPO_DIR%\icons\1600p.ico" (
        copy "%REPO_DIR%\icons\1600p.ico" "%ICON_PATH%" >nul 2>&1
    ) else (
        powershell -NoProfile -Command "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; Invoke-WebRequest -Uri '%GITHUB_BASE%/icons/1600p.ico' -OutFile '%ICON_PATH%' -UseBasicParsing"
    )
)
...
if not exist "%SETTINGS_LOCAL%" (
    if exist "%REPO_DIR%\GameUserSettings\1600p\GameUserSettings.ini" (
        copy "%REPO_DIR%\GameUserSettings\1600p\GameUserSettings.ini" "%SETTINGS_LOCAL%" >nul 2>&1
    ) else (
        echo Downloading 1600p GameUserSettings.ini...
        powershell -NoProfile -Command "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; Invoke-WebRequest -Uri '%GITHUB_BASE%/GameUserSettings/1600p/GameUserSettings.ini' -OutFile '%SETTINGS_LOCAL%' -UseBasicParsing"
    )
)
...
echo Fortnite 1600p session ended.
endlocal
